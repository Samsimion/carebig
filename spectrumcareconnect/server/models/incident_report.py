...
class IncidentReport(db.Model, SerializerMixin):
    ...
    severity = db.Column(db.Enum('low','medium','high','critical', name='incident_report_severity'))
    status = db.Column(db.Enum('open','reviewed','resolved','archived', name='incident_report_status'))
